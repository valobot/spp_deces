...
<body>
	...
	<table border="1" width="87%">
		...
		<td colspan="">Rp. </td>
		<td align='right'><?php $jumlahkan = "SELECT SUM(jumlah) AS total FROM pengeluaran_sklh where tanggal like '%$tanggal%'"; 
	...
	<table border="1" width="87%">
		...
		<td colspan="4" align="right"><b>TOTAL</b></td>
		<td colspan="">Rp. </td>
		<td align='right'><?php $jumlahkan = "SELECT SUM(jumlah) AS total FROM pengeluaran_yps where tanggal like '%$tanggal%'"; 
			$hasil =@mysqli_query($jumlahkan) or die (mysqli_error());
			$row = mysqli_fetch_array($hasil); 
