...
<body>
	<center>
		...
		<a href="?page=sppsmp"><b>[Back]</b></a><br>
	</center>
	<div id="div1" class="font">
	<font size="4"><b>REKAPITULASI PEMBAYARAN SUMBANGAN PENYELENGGARAAN PENDIDIKAN<br> SISWA/I SMP TUNAS BANGSA CILODONG (DECES)<br></font>
	<table border="1" width="94%" align="left">
		...
		$tgl= $_POST['tgl']; 
		$q = "SELECT * from spp_smp INNER JOIN siswa ON spp_smp.id=siswa.id INNER JOIN kelas ON spp_smp.id_kelas=kelas.id_kelas INNER JOIN jurusan ON spp_smp.id_jrsn=jurusan.id_jrsn where tgl like '%$tgl%' "; 
		...
		<?php 
			$no++; } ?>
			<tr>
				<td colspan="7" align="right"><b>Jumlah Pemasukan Hari Ini</b></td>
				<td colspan="">Rp. </td>
				<td align='right'><?php $jumlahkan = "SELECT SUM(jumlah_bayar) AS total FROM spp_smp where tgl like '%$tgl%'"; 
					$hasil =@mysqli_query($jumlahkan) or die (mysql_error());
