...
<body>
 <table width="97%" border='0'>
	<tr>
		<td>
			<form action="?page=detailsppsmp" method="POST" name="form1">
		...
		<td>
			<form action="?page=formsppsmp" method="POST" name="form1">
...
<table width="97%" border='0'>
 <tr>
	<td><center>
	<fieldset><legend><strong><b>Data Pembayaran Siswa SMP</b></strong></legend>
	<table width="96%" border="1" align="center" cellspacing="0">
		...
		$query="select * from spp_smp INNER JOIN siswa ON spp_smp.id=siswa.id INNER JOIN kelas ON spp_smp.id_kelas=kelas.id_kelas order by id_sppsmp desc limit $posisi,$batas";
	...
	<tr align="" bgcolor="#DFFDD5">
		...
		<td align="center">
			<a href="?page=edtsppsmp&&id_sppsmp=<?php echo $data['id_sppsmp']; ?>"> <img src="images/button-edit.gif" class="preview" title="Edit" width="20" height="20"></img></a> |
			<!--			
			<a href="?page=delsppsmp&&id_sppsmp=<?php echo $data['id_sppsmp'];?>" onClick="return confirm('Apakah Anda yakin akan menghapus Data ini ?...')"><img src="images/button-cross.gif" class="preview" title="Hapus" width="20" height="20"></img></a> |
			-->
			<a href="?page=kwitansismp&&id_sppsmp=<?php echo $data['id_sppsmp']; ?>"><img src="images/printer1.png" class="preview" title="Print" width="20" height="25"></img>
		</td>
	</tr>
	<?php 
	$no++; 
	...
	<?php		
	//=============PAGING ========================
		$sql_paging = mysqli_query("select id_sppsmp from spp_smp");
		...
		for($i = 1; $i <= $jumlah_halaman; $i++)
			if($i != $halaman) {
				echo "<a href=?page=sppsmp&&halaman=$i>&nbsp; $i &nbsp;</a><b>|</b>"; 
			} else {
				echo "<b> $i &nbsp; |</b>";
			}
		?>
